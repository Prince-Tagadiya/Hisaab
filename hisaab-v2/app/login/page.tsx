...
export default function LoginPage() {
  ...
  return (
    <div className="font-sans bg-[#f6f7f8] text-slate-900 min-h-screen flex flex-col antialiased overflow-hidden relative selection:bg-[#2b8cee]/20">
          ...
          <div className="hidden lg:flex flex-col gap-8 pr-12">
            ...
            <div className="grid grid-cols-2 gap-6 mt-4">
              <div className="flex flex-col gap-2 p-4 rounded-2xl bg-white border border-slate-100 shadow-sm">
                <Users className="text-[#2b8cee] mb-1" size={32} />
              ...
              <div className="flex flex-col gap-2 p-4 rounded-2xl bg-white border border-slate-100 shadow-sm">
                <RefreshCw className="text-[#2b8cee] mb-1" size={32} />
      ...
      <footer className="relative z-10 w-full py-6 text-center">
        <p className="text-xs text-slate-400">© 2024 Hisaab Inc. All rights reserved.</p>
      </footer>
    </div>
  );
}

function AuthCard({ view, setView, handleLogin }: { view: string, setView: any, handleLogin: any }) {
  return (
    <div className="w-full flex justify-center lg:justify-end">
      <motion.div 
        layout
...
function LoginView({ setView, handleLogin }: { setView: any, handleLogin: any }) {
  return (
    <motion.div 
    ...
    >
      ...
      <div className="mt-8 pt-6 border-t border-slate-100 text-center">
        <p className="text-xs text-slate-400 leading-relaxed px-4">
          ...
          <Link className="text-slate-600 hover:text-[#2b8cee] underline decoration-slate-300 underline-offset-2 ml-1" href="/terms">Terms of Service</Link> 
          and acknowledge our 
          <Link className="text-slate-600 hover:text-[#2b8cee] underline decoration-slate-300 underline-offset-2 ml-1" href="/privacy">Privacy Policy</Link>.
        </p>
      </div>
    </motion.div>
  );
...
function HelpView({ setView }: { setView: any }) {
  return (
    <motion.div 
      initial={{ opacity: 0, x: 20 }}
      animate={{ opacity: 1, x: 0 }}
    ...
    >
      ...
      <div className="space-y-3">
        ...
        >
          ...
          <ArrowRight className="text-slate-300 group-hover/item:text-[#2b8cee] transition-colors" size={20} />
        </button>
      ...
      <div className="text-center">
        <button onClick={() => setView('login')} className="inline-flex items-center gap-1.5 text-sm font-medium text-slate-500 hover:text-[#2b8cee] transition-colors group/link">
          <ArrowLeft className="group-hover/link:-translate-x-0.5 transition-transform" size={18} />
          Back to Sign In
        </button>
      </div>
    </motion.div>
...
function TroubleshootingView({ setView, handleLogin }: { setView: any, handleLogin: any }) {
  return (
    <motion.div 
      initial={{ opacity: 0, x: 20 }}
      animate={{ opacity: 1, x: 0 }}
    ...
    >
      ...
      <div className="space-y-4 mb-6">
        ...
        <div className="flex gap-4 p-4 rounded-xl bg-slate-50 border border-slate-100">
          <Cookie className="text-amber-500 mt-0.5 shrink-0" size={24} />
          <div className="space-y-1">
        ...
        <div className="flex gap-4 p-4 rounded-xl bg-slate-50 border border-slate-100">
          <History className="text-amber-500 mt-0.5 shrink-0" size={24} />
          <div className="space-y-1">
